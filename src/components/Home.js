...
export default function Home() {
  ...
  const baseURL = 'https://6497def49543ce0f49e177aa.mockapi.io/products?bestseller=true'
  console.log(baseURL)
  ...
  const truncateName = (name) => {
    const maxLength = 23; // Maximum characters allowed before truncation
        ...
        {APIData.map((item) => (
          <Grid item sm={12} md={4} key={item.id}>
            <Card sx={{ alignSelf: 'normal', borderRadius: '20px', display: 'flex', flexDirection: 'column' }} >
              <CardMedia
                sx={{ width: '100%', height: '300px', objectFit: 'scale-down' }}
